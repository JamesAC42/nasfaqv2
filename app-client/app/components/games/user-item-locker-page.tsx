...
import { fetchUserItemLocker } from "@/app/lib/games-api";
...
function ItemLockerCanvas({ items }: { items: GameItemLockerEntry[] }) {
  ...
  useEffect(() => {
    ...
    for (const bubble of bubbles) {
      ...
      bubble.image = image;
      image.onload = () => { bubble.imageLoaded = true; };
...
export function UserItemLockerPage({ username }: { username: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setError(null);
      try {
        const result = await fetchUserItemLocker(username);
      ...
      <section className={styles.page}>
        <div className={styles.topPanel}>
          <div className={styles.headingBlock}>
            <Link href={`/profile/${encodeURIComponent(username)}`} className={styles.backLink}>
              <FaArrowLeft />
              <span>Back to {username}&#39;s profile</span>
            </Link>
            <div className={styles.titleWrap}>
              <span className={styles.kicker}>Item Locker</span>
              <h1 className={styles.title}>{username}&#39;s Collection</h1>
            </div>
          </div>
        ...
        <div className={styles.canvasPanel}>
          {error ? (
            ...
            <div className={styles.stateCard}>
              ...
              <p>{username}&#39;s locker canvas is loading.</p>
            </div>
          ) : items.length ? (
            ...
            <div className={styles.stateCard}>
              ...
              <p>{username} hasn&apos;t collected any gacha hats or items yet.</p>
            </div>
          )}
        </div>
        ...
        <aside className={`${styles.drawer} ${isDrawerOpen ? styles.drawerOpen : ""}`.trim()} aria-label={`${username}'s won item list`}>
